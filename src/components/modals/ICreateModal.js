...
const ICreateModal = ({ isOpen, toggle, onSave, tutorId }) => {
  ...
  return (
    <Modal isOpen={isOpen} toggle={toggle}>
        <ModalHeader toggle={toggle}>Crear Informe</ModalHeader>
        <ModalBody>
            ...
            <Row>
                <Col md={6}>
                    <FormGroup>
                        <Label for="id_tipo_informe">Tipo de Informe</Label>
                        <Input 
                            type="select" 
                            name="id_tipo_informe" 
                            value={reportData.id_tipo_informe} 
                            onChange={handleInputChange}   
                        >
                            <option value="">Seleccionar tipo de informe</option>
                            {tiposInforme.map((tipo) => (
                            <option key={tipo.id_Tipo} value={tipo.id_Tipo}>{tipo.nombreTipo}</option>
                            ))}
                        </Input>
                    </FormGroup>
                </Col>
